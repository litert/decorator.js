...
export function compose(decorators: readonly ICallbackFn[]): ICallbackFn {
    ...
    iCompose.assertDecorators('GetterDecorators.compose', decorators);
 ...
 *     if (!GetterDecorators.validateArgs(args)) {
 *        throw new Error('Must be used as a member getter decorator!');
 *     }
 *     const [, ctx] = args;
 *     // Do something
 * }
 ...
 *
 * @param callback  The getter decorator callback function to be wrapped.
 * @returns  A new getter decorator function with the args check.
 * @throws {TypeError} If the arguments is invalid.
 *
 * @example
 * ```ts
 * const myDecorator = GetterDecorators.withArgsCheck((target, ctx) => {
 *     // Do something
 * });
 * ```
 */
