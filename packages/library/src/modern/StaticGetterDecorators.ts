...
export function compose(decorators: readonly ICallbackFn[]): ICallbackFn {
    ...
    iCompose.assertDecorators('StaticGetterDecorators.compose', decorators);
 ...
 *     if (!StaticGetterDecorators.validateArgs(args)) {
 *        throw new Error('Must be used as a static getter decorator!');
 *     }
 *     const [, ctx] = args;
 *     // Do something
 * }
 ...
 * will be thrown.
 *
 * @param callback  The static getter decorator callback function to be wrapped.
 * @returns  A new static getter decorator function with the args check.
 * @throws {TypeError} If the arguments is invalid.
 *
 * @example
 * ```ts
 * const myDecorator = StaticGetterDecorators.withArgsCheck((target, ctx) => {
 *     // Do something
 * });
 * ```
 */
