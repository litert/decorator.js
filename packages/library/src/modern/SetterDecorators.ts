...
export function compose(decorators: readonly ICallbackFn[]): ICallbackFn {
    ...
    iCompose.assertDecorators('SetterDecorators.compose', decorators);
 ...
 *     if (!SetterDecorators.validateArgs(args)) {
 *        throw new Error('Must be used as a member setter decorator!');
 *     }
 *     const [, ctx] = args;
 *     // Do something
 * }
 ...
 *
 * @param callback  The setter decorator callback function to be wrapped.
 * @returns  A new setter decorator function with the args check.
 * @throws {TypeError} If the arguments is invalid.
 *
 * @example
 * ```ts
 * const myDecorator = SetterDecorators.withArgsCheck((target, ctx) => {
 *     // Do something
 * });
 * ```
 */
