...
export function compose(decorators: readonly ICallbackFn[]): ICallbackFn {
    ...
    iCompose.assertDecorators('StaticSetterDecorators.compose', decorators);
 ...
 *     if (!StaticSetterDecorators.validateArgs(args)) {
 *        throw new Error('Must be used as a static setter decorator!');
 *     }
 *     const [, ctx] = args;
 *     // Do something
 * }
 ...
 * will be thrown.
 *
 * @param callback  The static setter decorator callback function to be wrapped.
 * @returns  A new static setter decorator function with the args check.
 * @throws {TypeError} If the arguments is invalid.
 *
 * @example
 * ```ts
 * const myDecorator = StaticSetterDecorators.withArgsCheck((target, ctx) => {
 *     // Do something
 * });
 * ```
 */
