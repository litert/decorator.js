...
export function compose(decorators: readonly ICallbackFn[]): ICallbackFn {
    ...
    iCompose.assertDecorators('StaticPropertyDecorators.compose', decorators);
 ...
 * will be thrown.
 *
 * @param callback  The static property decorator callback function to be wrapped.
 * @returns  A new static property decorator function with the args check.
 * @throws {TypeError} If the arguments is invalid.
 *
 * @example
 * ```ts
 * const myDecorator = StaticPropertyDecorators.withArgsCheck((value, ctx) => {
 *     // Do something
 * });
 * ```
 */
