...
NodeTest.describe('[Legacy] Static Method Decorators', () => {

    let result: IDict<boolean> = {};

    function testGeneral(pos: string): GeneralDecorators.ICallbackFn {
        return function (...args: any[]) {
            result[pos] = StaticMethods.validateArgs(args);
        };
    }

    function testDedicated(): StaticMethods.ICallbackFn {
    ...
    NodeTest.it('validateArgs should reject null descriptor arguments', () => {
        ...
        NodeAssert.strictEqual(
            StaticMethods.validateArgs([MyClass, 'method', null]),
            false,
        );
    });
    ...
    NodeTest.it('validateArgs should pass insides static method decorators', () => {
    ...
    NodeTest.it('decorators created by withArgsCheck should allows decorating static methods', () => {

        const testDedicate = StaticMethods.withArgsCheck(function (m, ctx) {
            void m;
            void ctx;
         });
    ...
    NodeTest.it('decorators created by withArgsCheck should not allow decorating non-static-method elements', () => {
    ...
    NodeTest.it('decorators created by create should allows decorating static methods', () => {

        let ctx: StaticMethods.IContext | null = null;
        let descriptor: PropertyDescriptor | undefined;
        let ctor: any;
        const testDedicate = StaticMethods.create(function (value) {
        ...
        NodeAssert.doesNotThrow(() => {
            ...
            ctor = MyClass;
            descriptor = Object.getOwnPropertyDescriptor(MyClass, 'method');
            void MyClass;
        });

        const actual = ctx as unknown as StaticMethods.IContext;
        NodeAssert.ok(actual);
        NodeAssert.ok(descriptor);
        NodeAssert.strictEqual(actual.type, EContextType.STATIC_METHOD);
        NodeAssert.strictEqual(actual.constructor, ctor);
        NodeAssert.strictEqual(actual.methodName, 'method');
        NodeAssert.strictEqual(actual.descriptor.value, descriptor.value);
    });

    NodeTest.it('decorators created by create should not allow decorating non-static-method elements', () => {

        const testDedicate = StaticMethods.create(function (ctx) {
