...
NodeTest.describe('[Legacy] Static Getter Decorators', () => {

    let result: IDict<boolean> = {};

    function testGeneral(pos: string): GeneralDecorators.ICallbackFn {
        return function (...args: any[]) {
            result[pos] = StaticGetters.validateArgs(args);
        };
    }

    function testDedicated(): StaticGetters.ICallbackFn {
    ...
    NodeTest.it('validateArgs should reject null descriptor arguments', () => {
        ...
        NodeAssert.strictEqual(
            StaticGetters.validateArgs([MyClass, 'value', null]),
            false,
        );
    });
    ...
    NodeTest.it('validateArgs should pass insides static getter decorators', () => {
    ...
    NodeTest.it('decorators created by withArgsCheck should allows decorating static getters', () => {

        const testDedicate = StaticGetters.withArgsCheck(function (ctor, propName, descriptor) {
            void ctor;
            void propName;
            void descriptor;
         });
        ...
        NodeAssert.doesNotThrow(() => {
            ...
            class MyClass {
                ...
                @testDedicate
                public static get value(): string { return '123'; }
            }
            void MyClass;
        });
    });

    NodeTest.it('decorators created by withArgsCheck should not allow decorating non-static-getter elements', () => {
    ...
    NodeTest.it('decorators created by create should allows decorating static getters', () => {

        let ctx: StaticGetters.IContext | null = null;
        let descriptor: PropertyDescriptor | undefined;
        const testDedicate = StaticGetters.create(function (value) {
        ...
        const actual = ctx as unknown as StaticGetters.IContext;
        NodeAssert.ok(actual);
        NodeAssert.ok(descriptor);
        NodeAssert.strictEqual(actual.type, EContextType.STATIC_GETTER);
        NodeAssert.strictEqual(actual.getterName, 'value');
        NodeAssert.strictEqual(actual.descriptor.get, descriptor.get);
    });

    NodeTest.it('decorators created by create should not allow decorating non-static-getter elements', () => {

        const testDedicate = StaticGetters.create(function (ctx) {
