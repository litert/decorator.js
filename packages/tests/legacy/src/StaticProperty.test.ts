...
NodeTest.describe('[Legacy] Static Property Decorators', () => {
    ...
    NodeTest.it('decorators created by withArgsCheck should allows decorating static properties', () => {

        const testDedicate = StaticProperties.withArgsCheck(function (proto, propName) { 
            void proto;
            void propName;
         });
    ...
    NodeTest.it('decorators created by create should allows decorating static properties', () => {

        let ctx: StaticProperties.IContext | null = null;
        let ctor: any;
        const testDedicate = StaticProperties.create(function (value) {
        ...
        const actual = ctx as unknown as StaticProperties.IContext;
        NodeAssert.ok(actual);
        NodeAssert.strictEqual(actual.type, EContextType.STATIC_PROPERTY);
        NodeAssert.strictEqual(actual.constructor, ctor);
        NodeAssert.strictEqual(actual.propertyName, 'a');
    });

    NodeTest.it('decorators created by create should not allow decorating non-static-property elements', () => {

        const testDedicate = StaticProperties.create(function (ctx) {
