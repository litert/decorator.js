...
import { EContextType, Methods, GeneralDecorators } from '@litert/decorator/legacy';
import { IDict } from '@litert/utils-ts-types';

NodeTest.describe('[Legacy] Method Decorators', () => {

    let result: IDict<boolean> = {};

    function testGeneral(pos: string): GeneralDecorators.ICallbackFn {
        return function (...args: any[]) {
            result[pos] = Methods.validateArgs(args);
        };
    }

    function testDedicated(): Methods.ICallbackFn {
    ...
    NodeTest.it('validateArgs should reject null descriptor arguments', () => {
        ...
        NodeAssert.strictEqual(
            Methods.validateArgs([MyClass.prototype, 'method', null]),
            false,
        );
    });
    ...
    NodeTest.it('decorators created by withArgsCheck should allows decorating methods', () => {

        const testDedicate = Methods.withArgsCheck(function (m, ctx) {
            void m;
            void ctx;
         });
    ...
    NodeTest.it('decorators created by withArgsCheck should not allow decorating non-method elements', () => {
    ...
    NodeTest.it('decorators created by create should allows decorating methods', () => {

        let ctx: Methods.IContext | null = null;
        let descriptor: PropertyDescriptor | undefined;
        const testDedicate = Methods.create(function (value) {
        ...
        const actual = ctx as unknown as Methods.IContext;
        NodeAssert.ok(actual);
        NodeAssert.ok(descriptor);
        NodeAssert.strictEqual(actual.type, EContextType.METHOD);
    ...
    NodeTest.it('decorators created by create should not allow decorating non-method elements', () => {

        const testDedicate = Methods.create(function (ctx) {
