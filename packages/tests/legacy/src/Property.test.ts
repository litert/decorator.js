...
import { EContextType, Properties, GeneralDecorators } from '@litert/decorator/legacy';
import { IDict } from '@litert/utils-ts-types';

NodeTest.describe('[Modern] Property Decorators', () => {

    let result: IDict<boolean> = {};

    function testGeneral(pos: string): GeneralDecorators.ICallbackFn {
        return function (...args: any[]) {
            result[pos] = Properties.validateArgs(args);
        };
    }

    function testDedicated(): Properties.ICallbackFn {
    ...
    NodeTest.it('validateArgs should pass insides member method decorators', () => {
    ...
    NodeTest.it('validateArgs should pass insides member property decorators', () => {
    ...
    NodeTest.it('decorators created by withArgsCheck should allows decorating properties', () => {

        const testDedicate = Properties.withArgsCheck(function (proto, propName) { 
            void proto;
            void propName;
         });
    ...
    NodeTest.it('decorators created by withArgsCheck should not allow decorating non-property elements', () => {
    ...
    NodeTest.it('decorators created by create should allows decorating properties', () => {

        let ctx: Properties.IContext | null = null;
        let proto: any;
        const testDedicate = Properties.create(function (value) {
        ...
        const actual = ctx as unknown as Properties.IContext;
        NodeAssert.ok(actual);
        NodeAssert.strictEqual(actual.type, EContextType.PROPERTY);
        NodeAssert.strictEqual(actual.constructor, proto.constructor);
        NodeAssert.strictEqual(actual.prototype, proto);
        NodeAssert.strictEqual(actual.propertyName, 'a');
    });

    NodeTest.it('decorators created by create should not allow decorating non-property elements', () => {

        const testDedicate = Properties.create(function (ctx) {
